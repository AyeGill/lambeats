...
(defcommand make-playlist (name song-list)
  "Makes a list of song objects into a playlist"
  (loop for song in song-list do
	(send "playlistadd" (wrap-quotes name) (wrap-quotes (file song)))))
...
(defun (setf sticker) (value connection type uri name)
  "Set the value of a sticker. value can be any printable lisp object: it is written in with the ~s directive of format in such a way that using sticker to get the value back will return the original lisp object"
  (set-sticker-* connection (from-keyword type) uri (from-keyword name) (format nil "~s" value))) 

(defcommand sticker (type uri name)
  (sticker-val (first (get-sticker-* connection (from-keyword type) uri (from-keyword name)))))

(defcommand get-sticker-* (type uri name)
  "Get the value of the sticker with the given name"
  (send "sticker get" type (wrap-quotes uri) name))

(defcommand set-sticker-* (type uri name value)
  "Set the value of the sticker with the given name"
  (send "sticker set" type (wrap-quotes uri) name (wrap-quotes value)))
